...
var XLSX = {};
 var  make_xlsx_lib=  function (XLSX){
XLSX.version = '0.17.5';
...
function excel_exprot (data, header, headerName, fileName) {
  ...
  const StrData = changeData(XLSX.write(workBook, wopts))
  fs.writeFile({
  	filePath: wx.env.USER_DATA_PATH + `/${fileName+time}.xlsx`, // wx.env.USER_DATA_PATH 指定临时文件存入的路径，后面字符串自定义
  	data: StrData,
  	...
  	success(res) {
  		...
  		wx.openDocument({
  			// 打开文档
  			filePath: wx.env.USER_DATA_PATH + `/${fileName+time}.xlsx`, //拿上面存入的文件路径
